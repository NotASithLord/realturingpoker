...
def send_support_email(ticket_id: str):
    ...
    if ticket.reported_by and ticket.reported_by.email:
        ...
        send_mail(email_subject, email_body, "support@turingpoker.com", [user_email])
...
def send_chips_email(email, from_username, amount, next_url):
    subject = f'You\'ve been given {amount} chips to play on Turing Poker!'
    signup_path = f'{reverse("Signup")}?{urlencode({"next": next_url})}'
    claim_chips_url = f'{settings.BASE_URL}{signup_path}'
    body =  f"""
        Hello friend.

        I've sent you {amount} chips to come and play poker on Turing Poker.
        ...
        {from_username}
    """
    send_mail(subject, body, "hello@turingpoker.com", [email])
